...
@tag
Feature: Setcampaign functionalities
  I want to use this template for my feature file

 @Regression 
  Scenario: Verify the set campaign functionality with valid details for a user who has enough rewards
    When Login to the application by a user who has an enough rewards and video uploaded.
    And  Naviagate to Mycontent from the side menu.
    And Click on Set campaign option of a video
...
#		Then Verify the rewards updated when campaign is activated
    ...
    And Click on Set campaign option of a video in mycontent
